...
class ShapeNetClassficationDataset(data.Dataset):
    def __init__(self,
            ...
            for file in filelist:
                ...
                if category in self.cat.values():
                    self.meta[self.id2cat[category]].append(os.path.join(self.root, category, uuid+'.txt'))
    ...
    def __getitem__(self, index):
        ...
        # print("cls",cls)
        point_set = np.loadtxt(fn[1]).astype(np.float32)[:, :3]
...
class ShapeNetSegmentationDataset(data.Dataset):
    def __init__(self,
        ...
        self.seg_label_offsets = dict(zip(self.seg_classes.keys(), np.cumsum([0] + list(self.seg_classes.values())[:-1])))
    ...
    def __getitem__(self, index):
        ...
        point_set = data[:, :3]
        seg = data[:, -1].astype(np.int64) - self.seg_label_offsets[fn[0]] + 1
